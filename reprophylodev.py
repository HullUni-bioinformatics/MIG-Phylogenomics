        ...
        for raxml_method in raxml_methods:
            ...
            for trimmed_alignment in raxml_method.command_lines.keys():
                ...
                if isinstance(raxml_method, RaxmlConf):
                    ...
                    elif raxml_method.preset == 'fd':
                        t = Tree('RAxML_bestTree.'+raxml_method.id+'_'+trimmed_alignment+'0')
...
def write_raxml_clines(tree_method, pj, trimmed_alignment_name):
    ...
    if len(tree_method.cline_args.keys()) > 0:
                    for key in tree_method.cline_args:
                        for c in presets[tree_method.preset]:
                            c[key] = tree_method.cline_args[key]
                        if tree_method.cline_args[key] == True:
                            for c in presets[tree_method.preset]:
                                c[key] = ''
